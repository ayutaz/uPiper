...
| zh | DotNetG2P.Chinese + ワークアラウンド (**部分移行**) | com.dotnetg2p.chinese | ⚠️ 追加修正必要 |
...
PR#47 で `LoadFromFile(string)` コンストラクタが public 化され、StreamingAssets 経由での辞書ロードが可能に。
uPiper 側で `ChineseG2PEngine(charPath, phrasePath)` を使った統合を実装済み。

**残課題**: `ToPuaPhonemes()` がトーンマーカーPUA (E046-E04A) を含まない。
uPiper 側でワークアラウンド実装済み（`ToIpaWithProsody()` の声調情報からトーンPUAを手動挿入）。
...
**現状**: PR#47 で `LoadFromFile` が public 化され、StreamingAssets 経由のロードが可能に。uPiper 側で統合実装済み。

**解決済み:**
- ✅ `ChineseG2PEngine(string charDictPath, string phraseDictPath)` コンストラクタで StreamingAssets ロード
- ✅ 文字カバレッジ 950 → 44,435文字（約47倍）、フレーズ多音字解消
- ✅ uPiper 側ワークアラウンドでトーンPUA挿入

**dot-net-g2p側の追加修正が必要な項目:**

1. **[必須] `ToPuaPhonemes()` にトーンマーカーPUA (E046-E04A) を含める**
   - 現在 `PinyinToPiperIpa.ConvertToPhonemes()` が「声調マーカーは含めない」設計
   - しかし piper-plus の実際の出力にはトーンPUA (`\ue046`-`\ue04a`) が各音節末尾に含まれる
   - `ToPuaPhonemes()` の出力に `tone1`-`tone5` のPUA文字を追加するか、
     トーン付き `ToPuaPhonemesWithTones()` メソッドを追加する
   - **現在 uPiper 側でワークアラウンド実装済み**: `ToIpaWithProsody()` の A1(声調) からトーンPUAを手動挿入

2. **[推奨] WebGL 非同期ロードパス**
   - `LoadFromStream(Stream)` を使って `WebGLStreamingAssetsLoader` 経由のロードに対応
