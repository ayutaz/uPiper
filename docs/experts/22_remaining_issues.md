...
### [解決済み] 3. dot-net-g2p 統合 — Phase 6 で完了

Phase 6 で英語・スペイン語・フランス語・ポルトガル語・韓国語の5言語をDotNetG2Pパッケージに移行完了。
MultilingualPhonemizerが各DotNetG2Pエンジン（EnglishG2PEngine, SpanishG2PEngine, FrenchG2PEngine, PortugueseG2PEngine, KoreanG2PEngine）を直接呼び出す。

**中国語のみ未移行**: ChinesePhonemizerBackend（Phase 5レガシー実装）を継続使用。
中国語トーンPUA問題のP1ワークアラウンドが適用済みのため、現時点では移行不要。
...
Phase 6 のDotNetG2P移行は完了（英語・スペイン語・フランス語・ポルトガル語・韓国語）。
中国語のみChinesePhonemizerBackend（レガシー）を継続使用。主な残課題は中国語トーンPUA（P1ワークアラウンド適用済み）。
