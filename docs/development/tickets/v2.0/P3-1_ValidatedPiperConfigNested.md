...
#### Config 三部作（P3-1/P3-3/P3-2）を単一チケットに統合すべきか

P3-1 → P3-3 → P3-2 は強い依存チェーンを持ち、同一エージェントが順次実施を推奨されている。統合の是非を検討した。

**統合しない判断の根拠**:
- 3 チケットの合計見積もりは 2.5 人日。単一チケットとしては十分管理可能なサイズだが、各チケットが独立したレビュー単位として機能する。P3-1 のネスト構造化を先にレビュー・マージすることで、P3-3 の副作用排除ロジックのレビュー時にはネスト構造が確定しており、認知負荷が下がる。
- P3-1 単体で独立した価値がある（22 プロパティの型レベル分類）。P3-3 も単体で独立した価値がある（副作用排除）。一方、P3-2 は P3-1/P3-3 なしでは冗長なインターフェース（21 プロパティ列挙）になるため単体価値が低い。
- 統合した場合、PR のレビュー範囲が `ValidatedPiperConfig.cs` の全面書き換え + `PiperConfig.cs` の Obsolete 化 + `IPiperConfigReadOnly.cs` 新規作成 + テスト 27 件移行 + 12 件新規と非常に大きくなる。

**統合を推奨する場合の条件**: もし M3 のスケジュールが逼迫し、個別 PR のレビューコストが並行性の利点を上回る場合、P3-1 + P3-3 を統合した「Config ネスト + 純粋関数化」チケットとし、P3-2 を別チケットとする 2 分割が最もバランスが良い。P3-1 と P3-3 は同じコンストラクタを変更するため、統合による実装効率向上が最も大きい。

#### Group B と Group A/C のファイル競合

Group B（Config 三部作）が変更するファイルと他グループの競合:

| Group B 変更ファイル | Group A 競合 | Group C 競合 |
|---------------------|-------------|-------------|
| `ValidatedPiperConfig.cs` | なし | なし |
| `PiperConfig.cs` | なし | なし |
| `TTSSynthesisOrchestrator.cs` | P2-1（シグネチャパススルー）、P2-2（Prosody 変数名） | P3-5（正規化呼び出し） |
| `PiperTTS.cs` | P2-1（ログ確認のみ、変更なし見込み） | なし |
| `IPiperConfigReadOnly.cs`（新規） | なし | なし |

**`TTSSynthesisOrchestrator.cs` が M3 全体の唯一のコンフリクトポイント**。3 グループ全てが変更するが、各グループの変更箇所は独立している:
- Group A: `PhonemeSilenceProcessor` パススルーのシグネチャ変更（P2-1）、`SynthesizeAsync` 内の Prosody 変数（P2-2）
- Group B: `_config?.Silence.ParsedPhonemeSilence` アクセスパス変更（P3-1）、コンストラクタ引数型変更（P3-2）
- Group C: `AudioNormalizer.NormalizeInPlace` 呼び出し変更（P3-5）

これらは行レベルで非重複であり、3-way マージで自動解決可能。

#### 相互参照の十分性

P3-1 のセクション 7 で P3-3 と P3-2 への引き継ぎ事項が記載されており、以下が明確:
- P3-3 への引き継ぎ: コンストラクタの record struct 初期化構造が確定済み
- P3-2 への引き継ぎ: 6 ネストプロパティ名が確定済み

不足している相互参照: **P2-1/P2-2 が P3-1 のネスト構造に与える影響の記載がない**。P2-1 は `PiperVoiceConfig`（`ValidatedPiperConfig` ではない）を変更するため直接の影響はないが、P3-2 で `PiperTTS._config` 参照を `_validatedConfig` 経由に移行する際に、P2-1 で変更された `PhonemeIdMap` の型を意識する必要がある。ただし P3-2 のスコープでは `PhonemeIdMap` を参照する箇所は移行対象外（`MultilingualPhonemizerOptions` 構築用）のため、実質的な影響はない。
