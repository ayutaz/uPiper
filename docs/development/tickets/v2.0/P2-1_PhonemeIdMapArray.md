...
#### P2-1 と P2-2 のデータモデル統合可能性

P2-1（PhonemeIdMap `int[]`）と P2-2（Prosody フラット配列化）は共に「piper-plus との互換性向上」を目的とするデータモデル変更であり、統合して単一チケットとすべきか検討した。

**統合しない判断の根拠**:
- P2-1 は `PiperVoiceConfig` のフィールド型変更（`Dictionary` の値型）が起点。P2-2 は `MultilingualPhonemizeResult` / `SynthesisRequest` 等 5 型のプロパティ構造変更が起点。変更の対象データ構造が完全に異なる。
- P2-1 の影響範囲は 19 ファイル（うち機械的変換が大半）、P2-2 は 35 ファイル（ロジック変更を含む）。統合すると 50+ ファイルの巨大 PR になり、レビュー・回帰確認が困難になる。
- P2-1 → P2-2 の順序依存は「P2-1 の型変更が P2-2 のテストコードに影響する」程度であり、同一エージェントが順次実施すれば十分。

**ただし、破壊的変更の一括リリースは必須**: P2-1 と P2-2 は共に Breaking Change であり、v2.0 リリースで同時に公開する必要がある。どちらかだけ先行リリースすると、外部利用者が中間状態の API に依存するリスクがある。M3 ゲートで両方の完了を確認してから beta リリースする設計は妥当。

#### グループ間のファイル競合分析

M3 では Group A（P2-1/P2-2）、Group B（P3-1/P3-3/P3-2）、Group C（P3-5/P3-4）が並行実行される。P2-1 が触るファイルと他グループの競合を分析した:

| P2-1 変更ファイル | Group B 競合 | Group C 競合 |
|------------------|-------------|-------------|
| `PiperVoiceConfig.cs` | なし | なし |
| `PhonemeEncoder.cs` | なし | なし |
| `PhonemeSilenceProcessor.cs` | なし | なし |
| `SplitInferenceOrchestrator.cs` | なし | なし |
| `TTSSynthesisOrchestrator.cs` | P3-1 がアクセスパス変更、P3-2 が型変更 | P3-5 が呼び出し変更 |
| `InferenceEngineDemo.cs` | なし | P3-5 が呼び出し変更 |
| `PiperTTS.cs` | P3-2 がフィールド型変更 | なし |

**要注意**: `TTSSynthesisOrchestrator.cs` は Group A（P2-1 でシグネチャ変更パススルー）、Group B（P3-1 でプロパティアクセスパス変更）、Group C（P3-5 で正規化呼び出し変更）の 3 グループ全てが変更する。ただし変更箇所（行）は重複しないため、マージ順序を守ればコンフリクトは自動解決可能。推奨マージ順序: **Group C → Group A → Group B**（Group B が最後なのは P3-2 の型変更が最も広範なため）。
