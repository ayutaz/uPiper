...
#### P2-1 との統合データモデルリファクタリング

P2-1（PhonemeIdMap `int[]`）と P2-2 は共に「piper-plus との型互換」が目的であり、一つのデータモデルリファクタリングチケットとして統合する案を検討した。

**統合しない判断の根拠**:
- 変更対象のデータ構造が完全に異なる。P2-1 は `PiperVoiceConfig.PhonemeIdMap` の辞書値型、P2-2 は Prosody の 5 型 15 プロパティの構造変更。共通の抽象化が存在しない。
- P2-2 単体で 35 ファイル・3 人日の規模がある。P2-1 と統合すると 50+ ファイルの巨大 PR になり、レビュー不能になる。
- P2-1 の変更はほぼ機械的（型変換）だが、P2-2 は stride=3 のインデックス計算やハンドラ戻り値型変更などロジック変更を伴う。作業の性質が異なる。

**破壊的変更の一括リリース戦略**: P2-1 と P2-2 は共に Breaking Change（public API の変更）。v2.0-beta リリース時に M3 ゲートで両方の完了を確認し、一括リリースする。段階的リリースは外部利用者に中間状態の API を公開するリスクがあり、不可。

#### グループ間のファイル競合分析

P2-2 は M3 の中で最大の影響範囲（35 ファイル）を持つ。他グループとの競合を分析した:

| P2-2 変更ファイル | Group B 競合 | Group C 競合 |
|------------------|-------------|-------------|
| `TTSSynthesisOrchestrator.cs` | P3-1（プロパティアクセス）、P3-2（型変更） | P3-5（正規化呼び出し） |
| `InferenceAudioGenerator.cs` | なし | P3-4（DetermineBackendType 削除） |
| `PhonemeSilenceProcessor.cs` | なし | なし |
| `PhonemeEncoder.cs` | なし | なし |
| `SplitInferenceOrchestrator.cs` | なし | なし |
| `PiperTTS.Inference.cs` | P3-2（フィールド型変更） | なし |

**要注意ファイル**:
1. **`TTSSynthesisOrchestrator.cs`**: 3 グループ全てが変更するが、変更箇所（行）は非重複。P2-2 は `SynthesizeAsync` 内の Prosody 変数名変更、P3-1 は Silence プロパティアクセス、P3-5 は正規化メソッド呼び出し。
2. **`InferenceAudioGenerator.cs`**: P2-2 は `GenerateAudioAsync` / `PrepareInputs` / `CreateProsodyTensorPooled` を変更。P3-4 は `DetermineBackendType` を削除。変更箇所が完全に独立しているためコンフリクトリスクは低い。

**推奨マージ順序**: Group C（P3-5/P3-4）→ Group A（P2-1/P2-2）→ Group B（P3-1/P3-3/P3-2）。Group A が最大規模のため、前後のグループが安定してからマージする方が安全。

#### P3-4（BackendSelector）との `InferenceAudioGenerator.cs` 同時変更

P2-2 と P3-4 は共に `InferenceAudioGenerator.cs` を変更するが、変更対象のメソッドが完全に独立している:
- P2-2: `GenerateAudioAsync`, `ExecuteInference`, `PrepareInputs`, `CreateProsodyTensorPooled`, `ExecuteWarmup`
- P3-4: `DetermineBackendType`（削除）、`InitializeAsync`（呼び出し変更）

この独立性は、`InferenceAudioGenerator` のメソッド粒度が適切であることを示している。ただし、P3-4 が先にマージされると P2-2 のリベース時に行番号がずれる（約 90 行の削除による）。P2-2 担当エージェントはリベース時に `InferenceAudioGenerator.cs` のコンフリクト解決に留意すること。
