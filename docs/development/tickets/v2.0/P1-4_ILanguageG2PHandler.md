...
**設計ドキュメント**: [P1-4_ILanguageG2PHandler.md](../../v2.0-design/P1-4_ILanguageG2PHandler.md)
...
| **DotNetG2P各エンジンのスレッドセーフティ** | DotNetG2P各エンジンの`Process()`呼び出しがスレッドセーフであるかが未確認 | 実装前に各エンジンのスレッドセーフティを確認。不安全な場合は`MultilingualPhonemizer.PhonemizeWithProsodyAsync`に`SemaphoreSlim(1,1)`を追加して排他制御を行う |
...
### 6.8 IL2CPP / WebGL 向け実装上の制約

- **CreateDefaultHandlerの実装では全7クラスをnewで静的にインスタンス化すること**（IL2CPPのmanaged code strippingでコード上の静的参照がないクラスが削除されるため）。link.xmlは不要
- **ILanguageG2PHandlerのXMLドキュメントに「WebGL対応のため、Process内でTask.Runを使用しないこと」を制約として明記すること**
...
- P2-2担当は本チケットのハンドラ実装に強く依存する。`ProcessXxx`の戻り値パターン `(string[] Phonemes, int[] A1, int[] A2, int[] A3)` を熟知していること