...
dot-net-g2pへの移行により、uPiperはネイティブライブラリを使用しなくなりました。P/Invoke設定は不要です。

ただしUnity.InferenceEngine（ONNX推論）内部のネイティブライブラリはUnityが管理するため、開発者による設定は不要です。
...
dot-net-g2pへの移行によりネイティブライブラリの手動配置は不要です。Unity.InferenceEngineのネイティブライブラリはUnityパッケージに含まれています。
...
1. **DllNotFoundException**
   - dot-net-g2pへの移行により、uPiper自体ではDllNotFoundExceptionは発生しません
   - Unity.InferenceEngineのネイティブライブラリに関する問題の場合は、Unityのバージョンとパッケージの再インストールを確認してください
...
```csharp
#if ENABLE_IL2CPP
    Debug.Log("[IL2CPP] Initializing uPiper...");
#endif
```
...
- [ ] link.xmlが配置されている
- [ ] Unity.InferenceEngineパッケージがインストールされている
...
dot-net-g2pへの移行により、P/Invokeは使用されなくなりました。uPiperの音素化処理は純粋C#実装（dot-net-g2p）で行われるため、ネイティブライブラリへのP/Invoke呼び出しは不要です。

Unity.InferenceEngine内部のネイティブライブラリはUnityが管理しており、開発者によるP/Invoke設定は不要です。
...
## 3. Unity AI Inference Engine (旧Sentis) の互換性
...
### 5.1 iOS
- dot-net-g2pは純粋C#のためネイティブリンク不要
- Info.plistへの設定追加が必要な場合あり

### 5.2 Android
- dot-net-g2pは純粋C#のためネイティブビルド不要

### 5.3 WebGL
- dot-net-g2pは純粋C#のためEmscriptenビルド不要（ただしWebGLは未対応）
