...
| # | タスク | 対象 | 工数 |
|---|--------|------|------|
| 1-1 | dot-net-g2p byte[]ロードAPI追加 | dot-net-g2p | 中 |
| 1-2 | Sentis 2.5.0へアップグレード | uPiper | 中 |
| 1-3 | link.xmlにdot-net-g2pアセンブリ追加 | uPiper | 小 |
| 1-4 | WebGLヒープサイズ拡大 (1024MB) | uPiper | 小 |
| 1-5 | StreamingAssets非同期ローダー作成 | uPiper | 中 |
| 1-6 | Task.Run → WebGL条件分岐 (13箇所) | uPiper + dot-net-g2p | 小 |

#### Phase 2: 日本語TTS WebGL動作（MVP達成）

| # | タスク | 依存 | 工数 |
|---|--------|------|------|
| 2-1 | DotNetG2PPhonemizerのWebGL初期化パス | 1-1, 1-5, 1-6 | 中 |
| 2-2 | CustomDictionaryのWebGL非同期ロード | 1-5 | 小 |
| 2-3 | PiperTTSのWebGLガード解除・統合 | 2-1, 2-2 | 中 |
| 2-4 | GPUPixel推論動作確認 | 1-2 | 小 |
| 2-5 | ユーザーインタラクションゲートUI | 2-3, 2-4 | 小 |
| 2-6 | WebGLビルド統合テスト | 全Phase 1-2 | 中 |

#### Phase 2b: 英語TTS対応

| # | タスク | 依存 | 工数 |
|---|--------|------|------|
| 2b-1 | CMUDictionary/FliteLTSのWebGL対応 | 1-5, 1-6 | 中 |
| 2b-2 | UnifiedPhonemizer/MixedLanguagePhonemizerのWebGL対応 | 2b-1 | 中 |

#### Phase 3: 最適化・配信

| # | タスク | 依存 | 工数 |
|---|--------|------|------|
| 3-1 | ZIP辞書圧縮ロード (103MB→23MB) | 2-1 | 中 |
| 3-2 | ONNXモデルFP16量子化 (60MB→30MB) | 2-4 | 中 |
| 3-3 | IndexedDBキャッシュ | 2-6 | 中 |
| 3-4 | GitHub Pages分割配信（過去実装移植） | 2-6 | 中 |
| 3-5 | 起動時プログレスバーUI | 2-5 | 小 |

#### Phase 4: WebGPU対応

| # | タスク | 依存 | 工数 |
|---|--------|------|------|
| 4-1 | Player Settings WebGPU有効化 | 1-2 | 小 |
| 4-2 | PlatformHelper.IsWebGPUプロパティ追加 | なし | 小 |
| 4-3 | DetermineBackendType() WebGPU判定追加 | 4-1, 4-2 | 小 |
| 4-4 | GPUComputeバックエンド動作検証 | 4-3, 2-6 | 中 |
| 4-5 | パフォーマンスベンチマーク | 4-4 | 中 |
