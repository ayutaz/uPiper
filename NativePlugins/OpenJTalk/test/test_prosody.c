...
// Test the new prosody API with A1/A2/A3 extraction
void test_prosody_api(void* openjtalk, const char* text) {
    printf("\n=== Testing NEW Prosody API for: \"%s\" ===\n", text);

    ProsodyPhonemeResult* result = openjtalk_phonemize_with_prosody(openjtalk, text);
    if (result) {
        printf("Phoneme count: %d\n", result->phoneme_count);

        printf("\nDetailed phonemes with A1/A2/A3 prosody:\n");
        printf("%-4s %-8s %-6s %-6s %-6s\n", "Idx", "Phoneme", "A1", "A2", "A3");
        printf("--------------------------------------\n");
        ...
        // Verify that prosody values are being extracted (at least some non-zero values)
        int non_zero_count = 0;
        for (int i = 0; i < result->phoneme_count; i++) {
            if (result->prosody_a1[i] != 0 || result->prosody_a2[i] != 0 || result->prosody_a3[i] != 0) {
                non_zero_count++;
            }
        }
        printf("\nProsody extraction validation:\n");
        printf("  Non-zero prosody values: %d / %d\n", non_zero_count, result->phoneme_count);
        if (non_zero_count > 0) {
            printf("  [PASS] Prosody values extracted successfully\n");
        } else {
            printf("  [WARN] No prosody values extracted (check label parsing)\n");
        }
...
// Legacy prosody test using PhonemeResult
...
int main(int argc, char* argv[]) {
    ...
    printf("\n========================================\n");
    printf("Testing LEGACY Prosody API (PhonemeResult)\n");
    printf("========================================\n");
    ...
    printf("\n========================================\n");
    printf("Testing NEW Prosody API (ProsodyPhonemeResult with A1/A2/A3)\n");
    printf("========================================\n");
    for (int i = 0; test_texts[i] != NULL; i++) {
        test_prosody_api(openjtalk, test_texts[i]);
    }
    ...
    printf("\n========================================\n");
    printf("All prosody tests completed\n");
    printf("========================================\n");
