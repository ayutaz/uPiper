...
namespace uPiper.Tests.Runtime.AudioGeneration
{
    ...
    public class ProsodyInferenceIntegrationTests
    {
        ...
        public async Task GenerateAudioWithProsody_ProducesValidAudio()
        {
            ...
            using var inferenceOutput = await _generator.GenerateAudioAsync(
                encodingResult.PhonemeIds,
                encodingResult.ExpandedProsodyFlat
            );
            var nativeAudioData = inferenceOutput.Audio;
        ...
        public async Task GenerateAudioWithProsody_CompareWithZeroProsody()
        {
            ...
            using var outputWithProsody = await _generator.GenerateAudioAsync(
                encodingResult.PhonemeIds,
                encodingResult.ExpandedProsodyFlat
            );
            ...
            using var outputWithoutProsody = await _generator.GenerateAudioAsync(
                encodingResult.PhonemeIds,
                zeroProsody
            );

            // Both should produce valid audio
            Assert.Greater(outputWithProsody.Audio.Length, 0,
                "Audio with prosody should have samples");
            Assert.Greater(outputWithoutProsody.Audio.Length, 0,
                "Audio without prosody should have samples");

            Debug.Log($"With prosody: {outputWithProsody.Audio.Length} samples " +
                $"({outputWithProsody.Audio.Length / 22050.0f:F2}s)");
            Debug.Log($"Without prosody: {outputWithoutProsody.Audio.Length} samples " +
                $"({outputWithoutProsody.Audio.Length / 22050.0f:F2}s)");
        ...
        public async Task ProsodyModel_InputTypes_MatchImplementationExpectations()
        {
            ...
            Assert.IsTrue(inferenceOutput.Audio.IsCreated,
                "Audio generation should succeed with Float prosody tensor");
            Assert.Greater(inferenceOutput.Audio.Length, 0, "Audio should have samples");

            Debug.Log($"[InputTypeTest] Successfully generated audio with Float prosody tensor: " +
                $"{inferenceOutput.Audio.Length} samples ({inferenceOutput.Audio.Length / 22050.0f:F2}s)");
        }
