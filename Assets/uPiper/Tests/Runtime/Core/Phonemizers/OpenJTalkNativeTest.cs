    ...
    public class OpenJTalkNativeTest
    {
        ...
        public void SetUp()
        {
            try
            {
                ...
                var dictPath = GetTestDictionaryPath();
                ...
                {
                    var filePath = System.IO.Path.Combine(dictPath, file);
                ...
                if (handle == IntPtr.Zero)
                {
                    ...
                    var errorCode = openjtalk_get_last_error(IntPtr.Zero);
                    var errorStrPtr = openjtalk_get_error_string(errorCode);
                    var errorStr = errorStrPtr != IntPtr.Zero ?
        ...
        {
            var text = "こんにちは";
            var resultPtr = openjtalk_phonemize(handle, text);

            Assert.AreNotEqual(IntPtr.Zero, resultPtr);

            var result = Marshal.PtrToStructure<PhonemeResult>(resultPtr);
            ...
            var phonemes = Marshal.PtrToStringAnsi(result.phonemes);
        ...
        public void TestErrorHandling()
        {
            ...
            var resultPtr = openjtalk_phonemize(handle, "");

            // Check if result is null or has no phonemes
            if (resultPtr == IntPtr.Zero)
            {
                var error = openjtalk_get_last_error(handle);
                Assert.AreNotEqual(0, error); // Should have an error

                var errorStrPtr = openjtalk_get_error_string(error);
                var errorStr = Marshal.PtrToStringAnsi(errorStrPtr);
        ...
        public void TestOptions()
        {
            ...
            // Test setting option
            var result = openjtalk_set_option(handle, "speech_rate", "1.5");
            Debug.Log($"openjtalk_set_option result: {result}, handle: {handle}");
            Assert.AreEqual(0, result); // 0 = OPENJTALK_SUCCESS

            // Test getting option
            var valuePtr = openjtalk_get_option(handle, "speech_rate");
        ...
        {
            var text = "今日は良い天気です";
            var iterations = 10; // Reduced from 100 to 10
        ...
        private string GetTestDictionaryPath()
        {
            ...
            var naistDictPath = System.IO.Path.Combine(
