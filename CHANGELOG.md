...
- **IPiperConfigReadOnly インターフェース**: `ValidatedPiperConfig` の読み取り専用インターフェース（P3-2）
  - `TTSSynthesisOrchestrator` / `PiperTTS` が具象クラスではなくインターフェースに依存するよう変更
  - `PiperTTS` の `_config` 直接参照を `_validatedConfig` 経由に安全移行（Performance, Audio, Inference, Silence）
